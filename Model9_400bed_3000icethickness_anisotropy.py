...
Amax = 321.4
#Amax = np.max(bumpfunc) #482.067176123
#print(Amax)
...
viscosityFnAir    = fn.misc.constant(1e12 / 3.1536e7)
...
# +
eta     = 1e19/ 3.1536e7  # viscosity
mu      = 10e9  # elastic modulus

alpha   = eta / mu                         # viscoelastic relaxation time
dt_e    = 20e3                      # elastic time step
...
viscousStressFn        = strainRateTensor * viscosityFn * 2.0 
tauHistoryFn           = eta_eff / ( mu * dt_e ) * previousStress
viscoelasticeStressFn  = viscousStressFn + tauHistoryFn

mappingDictStress = { materialA       : viscousStressFn, 
                      materialV       : viscousStressFn, 
                      materialVC     : viscousStressFn, 
                      materialR : viscoelasticeStressFn }
...
#meshVx = meshVz*MODEL_DATA['MAX_X']*(iceHeight-bed)/((iceHeight-bed)*MODEL_DATA['MAX_Z']-Sbump)
print(Sbump,meshVx) #4978220.286247666 0.1110573600552868
#meshVx = 0.25
...
velocityBCs = uw.conditions.DirichletCondition(
                                                ...
                                                indexSetsPerDof = (iWalls+base, base, kWalls+base),
                                              )
...
def flow(rotate_caxes = True):
    ...
    #phi = fn.misc.constant(dt/dt_e)
    phi = dt / dt_e;
    stressMapFn_data = stressMapFn.evaluate(swarm)
    #print(stressMapFn_data)
    previousStress.data[:] = (phi*stressMapFn_data[:] + (1.-phi)*previousStress.data[:])
    #previousStress.data[:] = phi*stress.data[:] + (1.-phi)*previousStress.data[:]
    #print(previousStress.data[:])
...
while step < maxSteps:
    ...
    if not step%stepsize: # if multiple of ..
        ...
        xdmf_previousStress = previousStress.save('previousStress_' + str(step) + '_'+ str(t) + '.h5')
        previousStress.xdmf('previousStress_' + str(step) + '.xdmf', xdmf_previousStress, 
                                "previousStress", xdmf_swarm, "Swarm", modeltime=step)

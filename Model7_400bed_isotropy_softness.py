...
viscosityFnAir    = fn.misc.constant(1e10 / 3.1536e7)
minViscosityIceFn  = fn.misc.constant(1e+13 / 3.1536e7)
maxViscosityIceFn  = fn.misc.constant(3.33e+15 / 3.1536e7)
viscosityFnRock = fn.misc.constant(1e18 / 3.1536e7)

V1 = 3*(1/90)*0.5 * (A0smallT * fn.math.exp(-QsmallT / (R*(particleTemperature + 273.)))) ** (-1./n) * (strainRate_2ndInvariantFn**((1.-n) / float(n)))
V2 = 3*(1/90)*0.5 * (A0highT * fn.math.exp(-QhighT / (R*(particleTemperature + 273.)))) ** (-1./n) * (strainRate_2ndInvariantFn**((1.-n) / float(n)))
V1 = V1 * 1e6 #pa*a
V2 = V2 * 1e6 #pa*a
